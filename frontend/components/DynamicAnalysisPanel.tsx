...
function SeverityBadge({ level }: { level: string }) {
  const colors: Record<string, string> = {
    CRITICAL: "bg-[rgba(244,63,94,0.12)] text-[#f43f5e] border-[#f43f5e]",
...
function Section({
  ...
  return (
    <div className="bg-[#050505] border border-[#1A1A1A] corner-brackets overflow-hidden">
      <button
      ...
      >
        ...
        <span className="text-[0.7rem] font-bold font-mono uppercase tracking-widest text-white">{title}</span>
        {count !== undefined && (
          <span className="ml-2 text-[0.6rem] font-mono text-[#555] bg-[#111] px-2 py-0.5 border border-[#222]">
            {count}
          </span>
        )}
      ...
      {open && <div className="px-5 pb-5 space-y-3 border-t border-[#111]">{children}</div>}
    </div>
  );
}
...
function ApiRow({ api, file, severity }: { api: string; file: string; severity: string }) {
  return (
    <div className="flex items-start gap-3 py-2 border-b border-[#0a0a0a] last:border-0">
      <SeverityBadge level={severity} />
      <div className="flex-1 min-w-0">
        <p className="text-[0.7rem] font-mono text-white font-bold truncate">{api}</p>
        <p className="text-[0.6rem] font-mono text-[#555] truncate">{file}</p>
      </div>
    </div>
  );
}
  ...
  if (!dynamic && !mobsf) {
    return (
      <div className="space-y-6">
        <div className="bg-[#050505] border border-[#1A1A1A] p-8 corner-brackets flex flex-col items-center gap-4 text-center">
          <Box className="w-12 h-12 text-[#222]" />
          <p className="text-[0.75rem] font-mono text-[#555] uppercase tracking-widest">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-[#050505] border border-[#1A1A1A] p-5 corner-brackets">
          <p className="text-[0.6rem] font-mono text-[#555] uppercase tracking-widest mb-2">Frida Sandbox</p>
          {dynamic?.sandbox_available ? (
            <>
              ...
              <p className="text-[0.6rem] font-mono text-[#555]">
            ...
            <>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[0.65rem] font-mono text-[#f43f5e] uppercase font-bold">Unavailable</span>
              </div>
              <p className="text-[0.6rem] font-mono text-[#555] truncate">{dynamic?.error ?? "Docker not running"}</p>
            </>
          )}
        </div>
        ...
        {dynamic?.behavioral_score && (
          <div className="bg-[#050505] border border-[#1A1A1A] p-5 corner-brackets">
            <p className="text-[0.6rem] font-mono text-[#555] uppercase tracking-widest mb-2">Behavioral Score</p>
            <div className="flex items-end gap-2">
              <span className="text-4xl font-black text-white font-mono">
                {dynamic.behavioral_score.score}
              </span>
              <span className="text-[#555] font-mono text-sm mb-1">/100</span>
              <SeverityBadge level={dynamic.behavioral_score.level} />
            </div>
            <div className="mt-2 w-full h-1 bg-[#111]">
              <div
        ...
        {/* MobSF Status */}
        <div className="bg-[#050505] border border-[#1A1A1A] p-5 corner-brackets">
          <p className="text-[0.6rem] font-mono text-[#555] uppercase tracking-widest mb-2">MobSF Deep Scan</p>
          {mobsf?.available ? (
            <>
              <div className="flex items-center gap-2 mb-1">
                <span className="w-2 h-2 rounded-full bg-[#4ade80] animate-pulse" />
            ...
            <>
              ...
              <p className="text-[0.6rem] font-mono text-[#555] truncate">{mobsf?.error ?? "MobSF container not running"}</p>
            </>
          )}
        </div>
      </div>
          ...
          <div className="mt-3 space-y-2">
            {dynamic.behavioral_score.flags.map((flag, i) => (
              <div key={i} className="flex items-start gap-2 text-[0.65rem] font-mono">
                <span className="text-[#f43f5e] mt-0.5 shrink-0">▶</span>
        ...
        >
          <div className="mt-3 space-y-1">
            ...
            {dynamic.smali_analysis.critical_apis.length === 0 &&
             dynamic.smali_analysis.high_apis.length === 0 && (
              <p className="text-[0.65rem] font-mono text-[#555] py-2">No dangerous API calls detected.</p>
            )}
          </div>
        </Section>
      )}
        ...
        <Section title="Cryptography Usage" icon={Lock} count={Object.keys(dynamic.smali_analysis.crypto_usage).length}>
          <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3">
            {Object.entries(dynamic.smali_analysis.crypto_usage).map(([algo, files]) => (
              ...
              >
                <p className="text-sm font-bold font-mono text-white">{algo}</p>
                <p className="text-[0.6rem] font-mono text-[#555] mt-1">{files.length} file(s)</p>
            ...
            {Object.entries(dynamic.smali_analysis.antianalysis).map(([technique, instances]) => (
              <div key={technique} className="border border-[#111] p-3">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[0.65rem] font-bold font-mono text-[#f43f5e] uppercase">
                ...
                {instances.slice(0, 2).map((inst, i) => (
                  <p key={i} className="text-[0.6rem] font-mono text-[#555] truncate">
                    {inst.file}: <span className="text-[#777]">{inst.match}</span>
                  </p>
                ))}
              </div>
            ))}
        ...
        <Section title="Sensitive Data in Code" icon={Shield} count={Object.keys(dynamic.smali_analysis.sensitive_data).length} defaultOpen={false}>
          <div className="mt-3 space-y-3">
            {Object.entries(dynamic.smali_analysis.sensitive_data).map(([dtype, instances]) => (
              <div key={dtype} className="border border-[#f43f5e]/20 bg-[rgba(244,63,94,0.03)] p-3">
                <p className="text-[0.65rem] font-bold font-mono text-[#f43f5e] uppercase mb-2">
                  {dtype.replace(/_/g, " ")}
                </p>
                {instances.slice(0, 3).map((inst, i) => (
                  <div key={i} className="text-[0.6rem] font-mono text-[#555] mb-1">
                    <span className="text-[#777]">{inst.file}:</span>{" "}
                    <span className="text-[#f43f5e]/80 font-bold">{inst.snippet.slice(0, 80)}</span>
                  </div>
                ))}
              </div>
            ))}
            ...
            {dynamic!.smali_analysis!.dynamic_loading.length > 0 && (
              <div>
                ...
                {dynamic!.smali_analysis!.dynamic_loading.slice(0, 5).map((f, i) => (
                  <p key={i} className="text-[0.6rem] font-mono text-[#555] truncate py-0.5">{f}</p>
                ))}
              </div>
            )}
            {dynamic!.smali_analysis!.native_libs.length > 0 && (
              <div>
                ...
                {dynamic!.smali_analysis!.native_libs.map((lib, i) => (
                  <p key={i} className="text-[0.65rem] font-mono text-white py-0.5">{lib}</p>
                ))}
              </div>
            )}
          </div>
        ...
        <Section title="Hardcoded Network Endpoints" icon={Globe} count={dynamic.smali_analysis.network_endpoints.length} defaultOpen={false}>
          <div className="mt-3 space-y-1">
            {dynamic.smali_analysis.network_endpoints.slice(0, 20).map((url, i) => (
              <p key={i} className="text-[0.65rem] font-mono text-[#22d3ee] truncate py-0.5 border-b border-[#0a0a0a]">
                {url}
              </p>
            ))}
          </div>
            ...
            {mobsf.findings.slice(0, 20).map((finding, i) => (
              <div key={i} className="border border-[#1A1A1A] p-3">
                <div className="flex items-start gap-2 mb-1">
                  ...
                  <p className="text-[0.65rem] font-mono text-white font-bold uppercase">{finding.title}</p>
                </div>
                {finding.desc && (
                  <p className="text-[0.6rem] font-mono text-[#555] mt-1">{finding.desc}</p>
                )}
